...
@section('page-content')
                    ...
                    <ul class="nav nav-tabs nav-tabs-space-lg nav-tabs-line nav-tabs-bold nav-tabs-line-3x" role="tablist">
                        <li class="nav-item mr-3">
                            <a class="nav-link active" data-toggle="tab" href="#kt_apps_projects_view_tab_2">
                                ...
                                <span class="nav-text font-weight-bold">Realtors</span>
                            </a>
                        </li>
                        <li class="nav-item mr-3">
                            <a class="nav-link" data-toggle="tab" href="#kt_apps_projects_view_tab_5">
                                ...
                                <span class="nav-text font-weight-bold">Property-Realtor Assignments</span>
                            </a>
                        </li>
                        <li class="nav-item mr-3">
                ...
                <div class="tab-content">
                    ...
                    <div class="tab-pane active" id="kt_apps_projects_view_tab_2" role="tabpanel">
                        <div class="table-responsive">
                            <table class="table table-vertical-center dataTable js-exportable" id="agents-table">
                                <thead>
                                    <tr class="text-left">
                                        ...
                                        <th class="pr-0">Phone Secondary</th>
                    ...
                    <div class="tab-pane" id="kt_apps_projects_view_tab_5" role="tabpanel">
                        <div class="row mb-2">
                          <div class="col-4">
                                <label for="">Realtors</label>
                                <select id="agent-id" class="form-control select2">
                        ...
                        <div class="table-responsive">
                            <table class="table table-vertical-center dataTable js-exportable" id="property-agents-table">
                                <thead>
                                    <tr class="text-left">
                                        <th style="width: 5px">No.</th>
                                        <th class="pl-0">Realtor Name</th>
    ...
    @include('admin.agents.modals.add')
    @include('admin.agents.modals.edit')
    @include('admin.agents.modals.view')
    @include('admin.agents.modals.assign-property')
    @include('admin.agents.modals.update-assign-property')
    ...
    <script>
                ...
                },
                {
                    ...
                    'render': function(data, type, full, meta) {
                        ...
                        if ("{!! Auth::user()->role !!}" == 'user') {
                            html += `<a style="cursor:pointer" class="btn btn-icon btn-light btn-hover-primary btn-sm mx-3 ping-btn">
        ...
        var propertyAgentsTable = $('#property-agents-table').DataTable({
            dom: "Bfrtip",
            ajax: {
                url: `${APP_URL}/api/agents/properties/${agentID}`,
                type: "GET"
            ...
            buttons: [{
                ...
                {
                    text: "Assign property to Realtor",
                    attr: {
                        class: "ml-2 btn-success btn btn-sm rounded"
                    },
                    action: function(e, dt, node, config) {
        ...
        $('#filter1').click(function() {
            ...
            propertyAgentsTable.ajax.url(
                    `${APP_URL}/api/agents/properties/${agentID}`
                )
                .load();
        })
        ...
        $("#agents-table").on("click", ".delete-btn", function() {
            var data = agentsTable.row($(this).parents("tr")).data();
            ...
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    fetch(`${APP_URL}/api/agents/delete`, {
                        method: "POST",
                        body: formdata,
                    }).then(function(res) {
                        return res.json()
                        ...
                        Swal.fire({
                            text: "Realtor deleted successfully",
                            icon: "success"
                        });
        ...
        $("#property-agents-table").on("click", ".delete-btn", function() {
            var data = propertyAgentsTable.row($(this).parents("tr")).data();
            var formdata = new FormData()
            formdata.append("transid", data.transid);
            formdata.append("createuser", CREATEUSER);
            ...
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    fetch(`${APP_URL}/api/agents/delete_agent_property`, {
                        method: "POST",
                        body: formdata,
                    }).then(function(res) {
                        return res.json()
                        ...
                        Swal.fire({
                            text: "Realtor property deleted successfully",
                            icon: "success"
                        });
        ...
        $("#trash-table").on("click", ".restore-btn", function() {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    fetch(`${APP_URL}/api/agents/restore`, {
                        method: "POST",
                        body: formdata,
                    }).then(function(res) {
                        return res.json()
                        ...
                        Swal.fire({
                            text: "Realtor restored successfully",
                            icon: "success"
                        });
                        trashTable.ajax.reload(false, null);
                        agentsTable.ajax.reload(false, null);
        ...
        $("#agents-table").on("click", ".update-btn", function() {
            let data = agentsTable.row($(this).parents('tr')).data();

            $("#update-modal").modal("show");
            $("#update-transid").val(data.transid);
            $("#update-userid").val(data.userid);
            $("#update-firstname").val(data.firstName);
            $("#update-lastname").val(data.lastName);
            $("#update-middlename").val(data.middleName);
            $("#update-email").val(data.email);
            $("#update-phone").val(data.phoneNumber);
            $("#update-phone2").val(data.phoneNumber2);
            $("#update-status").val(data.approved).trigger('change');         
        });

        $("#property-agents-table").on("click", ".update-btn", function() {
            let data = propertyAgentsTable.row($(this).parents('tr')).data();

            $("#update-assign-property-modal").modal("show");
            $("#update-transid1").val(data.transid);
            $("#update-owner-dropdown").val(data.ownerID).trigger('change');
            $("#update-status1").val(data.status).trigger('change');
            $("#update-agents-dropdown").val(data.agentID).trigger('change');
            $("#update-property-dropdown").val(data.propertyID).trigger('change');
       });

        $("#agents-table").on("click", ".view-btn", function() {
            let data = agentsTable.row($(this).parents('tr')).data();

            $("#view-modal").modal("show");
            document.getElementById("view-userid").innerHTML = data.userid;
            document.getElementById("view-name").innerHTML = data.name;
            document.getElementById("view-phone").innerHTML = data.phoneNumber;
            document.getElementById("view-phone2").innerHTML = data.phoneNumber2;
            document.getElementById("view-email").innerHTML = data.email;
            document.getElementById("view-approvalStatus").innerHTML = data.approvalStatus;
            document.getElementById("view-picture").innerHTML = data.picture;
        });
        ...
        $("#agents-table").on("click", ".ping-btn", function() {
            var data = agentsTable.row($(this).parents("tr")).data();
            var formdata = new FormData()
            formdata.append("createuser", CREATEUSER);
            formdata.append("ownerID", "{!! Auth::user()->userid !!}");
            formdata.append("agentID", data.userid);

            Swal.fire({
                title: "",
                text: `Proceed to invite to ${data.name}?`,
                icon: "confirm",
                showCancelButton: true,
                confirmButtonColor: "#DD6B55",
                ...
                if (result.value) {
                    ...
                    fetch(`${APP_URL}/api/agents/invite`, {
                        method: "POST",
                        body: formdata,
                    }).then(function(res) {
                        return res.json()
                        ...
                        if (err) {
                            Swal.fire({
                                text: "Sending invitation failed"
                            });
                        }
                    })
                }
