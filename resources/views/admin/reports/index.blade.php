...
@section('page-content')
                            ...
                            <table class="table table-vertical-center dataTable js-exportable" id="transfers-table">
                                <thead>
                                    <tr class="text-left">
                                        ...
                                        <th class="pr-0">Previous Owner</th>
                                        <th class="pr-0">New Owner</th>
                                        <th class="pr-0">Transfer Date</th>
                                        <th class="pr-0">Tranferred By</th>
    ...
    <script>
        ...
        var transfersTable = $('#transfers-table').DataTable({
            ...
            columns: [{
                ...
                },
                {
                    data: null,
                    'render': function(data, type, full, meta) {
                        return `<div class="text-dark-75 font-weight-bolder font-size-lg mb-0">${data.previousOwnerName}</div>
                        <span style="width: 108px;"><div class="font-weight-bolder text-dark mb-0">#${data.previousOwnerName}</div></span>`
                    }
                },
                {
                    data: null,
                    'render': function(data, type, full, meta) {
                        ...
                        <span style="width: 108px;"><div class="font-weight-bolder text-dark mb-0">#${data.newOwnerID}</div></span>`
                    }
                },
                {
                    data: "transferDate",
                },
                {
                    data: null,
                    'render': function(data, type, full, meta) {
                        return `<div class="text-dark-75 font-weight-bolder font-size-lg mb-0">${data.transferByName}</div>
                        <span style="width: 108px;"><div class="font-weight-bolder text-dark mb-0">#${data.createuser}</div></span>`
                    }
                },
                {
                    data: null,
        ...
        let propertyDropdown = document.getElementById("property-dropdown");
        let previousOwnerDropdown = document.getElementById("previous-owner-dropdown");
       
        previousOwnerDropdown.addEventListener("change", function(event) {
            let selectedIndex = previousOwnerDropdown.options.selectedIndex;
            let previousOwner = previousOwnerDropdown.options[selectedIndex].value;
            getProperties(null, previousOwner, propertyDropdown);
        });

        function getProperties(propertyID, previousOwner, propertyDropdown) {
            let selectedOwner = users.find((user) => user.userid == previousOwner);
            let fragment = new DocumentFragment();

            selectedOwner.properties.map(property => {
                let selected = property.transid == propertyID ? true : false;
                let option = new Option(property.title, property.transid, selected, selected);
                fragment.appendChild(option);
            });

            propertyDropdown.innerHTML = null;
            propertyDropdown.appendChild(fragment);
        }
